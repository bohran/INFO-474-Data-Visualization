(function() {
  ...
  function bestFitLine(data) {
    let greScores = data.map((row) => parseInt(row["GRE Score"]));
    let admissionRates = data.map((row) => parseFloat(row["Chance of Admit"]));
    let reg = linearRegression(greScores, admissionRates);
    console.log("a" + reg.a + "b" + reg.b);
    minMax = findMinMax(data);
    lxmin = minMax.xMin;
    lxmax = minMax.xMax; 

    lymin = reg.b + reg.a * lxmin;
    lymax = reg.b + reg.a * lxmax; 

    lxMin = (lxmin - 285) * 6 + 50; 
    lyMin = 450 - (lymin - 0.3) * 500;

    lxMax = (lxmax - 285) * 6 + 50; 
    lyMax = 450 - (lymax - 0.3) * 500;
