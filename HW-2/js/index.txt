(function() {
  ...
  function drawAxesLabels() {
    context.fillText("TOEFL Score", 170, 485);
    context.save();
    context.translate(10, 250);
    context.rotate(-Math.PI/2);
    context.fillText("Chance of Admittance", -50, 0);
    context.restore();
  }
  ...
  function drawAxesTicks(axesLimits) {
    ...
    for (let x = xMark; x < 400; x += 30) {
    // for (let x = xMark-2; x < 400; x += 30) {
  ...
  function plotPoint(x, y) {
    ...
    context.fillStyle = '#D3D3D3'; // changed color to light gray
    context.fill();
